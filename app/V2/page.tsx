          ...
          {STATS.map((s, i) => (
            <div
              ...
              className={[
                ...
                // mobile: right-column items get a left border
                i % 2 === 1 ? "border-l border-gray-200" : "",
                // mobile: bottom-row items get a top border, removed on desktop
                i >= 2 ? "border-t md:border-t-0 border-gray-200" : "",
                // desktop: all items after first get a left border
          ...
          {DIVISIONS.map((div, i) => (
            <FadeIn key={div.name} delay={i * 55} className="h-full">
              <div className="group relative border border-gray-200 rounded-xl p-6 flex flex-col gap-3 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg cursor-default h-full overflow-hidden">
                {/* Orange top accent slides in on hover */}
...
const MILESTONES = [
  ...
  {
    year: "2021",
    description: "Second manufacturing unit got WHO GMP Certification",
  },
        ...
        <FadeIn className="hidden md:block overflow-x-auto pb-4">
          <div className="flex min-w-max px-4">
            {MILESTONES.map((m, i) => (
              ...
              >
                <p className="font-barlow font-black text-brand text-xl mb-3">
                  {m.year}
                </p>
                {/* Dot + connecting line */}
                <div className="flex items-center w-full mb-4">
                  <div
                    className={`h-px flex-1 bg-gray-300 ${i === 0 ? "opacity-0" : ""}`}
                  />
                  <div className="w-3 h-3 rounded-full bg-brand border-2 border-[#F9FAFB] shadow-sm flex-shrink-0" />
                  <div
                    className={`h-px flex-1 bg-gray-300 ${i === MILESTONES.length - 1 ? "opacity-0" : ""}`}
                  />
                </div>
            ...
            <div key={m.year} className="flex gap-5">
              <div className="flex flex-col items-center pt-1">
                ...
                {i < MILESTONES.length - 1 && (
                  <div
                    className="w-px bg-gray-300 flex-1 mt-2"
                    style={{ minHeight: "40px" }}
                  />
                )}
              </div>
              <div className="pb-8">
                <p className="font-barlow font-black text-brand text-[18px] leading-none mb-1.5">
                  {m.year}
                </p>
                <p className="font-inter text-[14px] text-navy/70 leading-[1.65]">
                  {m.description}
                </p>
              </div>
            </div>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
          <div>
            ...
            <p className="font-inter text-[14px] leading-[1.85] text-white/50">
              507, Block-B, Elante Office Tower,
              <br />
              Plot 178-178A, Industrial Area, Phase-1,
              <br />
              Chandigarh 160002
            </p>
          </div>
          <div>
            ...
            <p className="font-inter text-[14px] leading-[1.85] text-white/50">
              D-904, 905, 906, &ldquo;The First&rdquo;,
              <br />
              Nr. Keshavbaug Party Plot, Vastrapur,
              <br />
              Ahmedabad 380015
            </p>
          </div>
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-6">
            <Link
              href="#"
              className="font-inter text-[12.5px] text-white/30 hover:text-white/60 transition-colors"
            >
              Terms of Service
            </Link>
            <Link
